...
**Spending rule: all GPU choices and dollar figures below are ideas and
estimates, not decisions. Aden has approved none of them. Any GPU rental
(Phases 1-3) needs his explicit sign-off at the point of usage, with the
card, hours, and cost stated up front. Phase 0 is the only pre-approved
work because it is free.**
...
### Phase 1: prototype (rented 5090s, est. ~$200-400 AUD total; requires Aden's go-ahead before any rental)
...
### Phase 2: serious (5090 or A100 PCIe, est. ~$150-250 AUD/run, 8-15 runs; each run individually authorized)
...
### Phase 3: final (2x H100 NVL DDP, est. ~$1,000-1,500 AUD, 1-2 runs; explicitly authorized by Aden)
...
- No decision on total budget has been made. The scenarios below are
  reference points from the cost analysis, nothing more.
- Reference scenarios: BALANCED ~$2,700 USD total, LEAN ~$1,300.
